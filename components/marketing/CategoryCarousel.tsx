...
export function CategoryCarousel() {
  ...
  return (
    <section
      className="text-ivory relative isolate overflow-hidden py-14 sm:py-16 lg:py-20"
      style={{
        // Deep navy, layered rather than flat: a vertical wash that lifts
        // towards the middle so the cards sit on light rather than on a slab.
        //
        // FIXED NAVY, NOT A SEMANTIC TOKEN. The band is dark in both themes by
        // design — like the video section below it — so it cannot follow
        // --background, which is ivory in light mode. Everything layered on top
        // is therefore pinned to light-on-dark values too, and none of it may be
        // swapped back to a semantic token without turning invisible.
        backgroundColor: "var(--color-navy)",
        backgroundImage: "linear-gradient(180deg, #061726 0%, var(--color-navy) 42%, #0d2c49 100%)",
      }}
    >
      ...
      <span
        ...
        style={{
          // Ivory threads now, not navy: navy-on-navy is nothing at all.
          //
          ...
          backgroundImage:
            "repeating-linear-gradient(58deg, var(--color-ivory) 0 1px, transparent 1px 14px), repeating-linear-gradient(-58deg, var(--color-ivory) 0 1px, transparent 1px 17px)",
        }}
      />
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div className="max-w-2xl">
            {/* Gold, not the burgundy this used on ivory — burgundy on navy is
                barely above the background. */}
            <p className="text-premium mb-3 text-xs font-semibold tracking-[0.2em] uppercase">
              Product Categories
            </p>
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              // Pinned to ivory-on-navy rather than bg-card: the band is dark in
              // both themes, and --card is navy in the dark theme, which would
              // make the control vanish into its own background.
              style={{
                backgroundColor: "var(--color-ivory)",
                color: "var(--color-navy)",
                borderColor: `color-mix(in srgb, ${activeAccent} 55%, transparent)`,
              }}
              className="glow-ring-blue hover:bg-premium size-11 rounded-full transition-colors"
            >
        ...
        >
          ...
          <span
            ...
            style={{
              // Ivory, not the category accent. Half the accents are dark
              // jewel tones — burgundy, forest, oxblood — and a dark numeral on
              // navy is simply not there. Gold on the stroke keeps it branded.
              color: "rgb(250 246 239 / 0.07)",
              WebkitTextStroke: "1px color-mix(in srgb, var(--color-accent-gold) 26%, transparent)",
            }}
          >
            ...
            return (
              ...
              >
                <Link
                  ...
                  // `bg-white text-navy` are not decoration, they are what makes
                  // the card readable on this band. The section sets text-ivory
                  // for its own heading, and that inherits straight into the
                  // card — ivory type on a pale card is invisible. glass-light
                  // is also translucent by design, which let the navy bleed
                  // through and turned every card muddy grey.
                  className={`group glass-light glass-glow focus-visible:ring-ring text-navy flex h-full flex-col rounded-xl bg-white p-5 transition-transform duration-300 focus-visible:ring-2 focus-visible:ring-offset-2 sm:p-6 ${
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{
                    width: isActive ? 30 : 8,
                    // The inactive dot was mixed from --foreground, which is
                    // navy in the light theme — invisible on this band.
                    backgroundColor: isActive ? category.accent : "rgb(250 246 239 / 0.28)",
                    boxShadow: isActive
